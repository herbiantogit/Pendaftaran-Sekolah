...
<th style="width: 5%; text-align: center; vertical-align: middle"> No </th>
    <th style="text-align: center; vertical-align: middle"> Nama Jenis keluarga </th>
        <th style="width: 20%; text-align: center; vertical-align: middle">
            <button id="tambah" data-target="#tambah_provinsi" data-toggle="modal" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i> Tambah</button>
        </th>
    </tr>
</thead>
    <tbody>
                ...
                <div id="tambah_provinsi" class="modal fade" tabindex="-1" aria-hidden="true">
...
<div class="col-md-12">
    <div class="form-group">
        <label class="control-label col-md-3">Nama Jenis keluarga</label>
            ...
            <input type="text" id="nama_provinsi" name="nama_provinsi" class="form-control" placeholder="Masukkan Nama Jenis keluarga" autocomplete="off" autofocus/>
        </div>
</div>
</div>
</div>
    ...
    icon.removeClass("fa-warning");
                            ...
                            submitHandler: function (form) {
                                ...
                                $.ajax({
                                    method: 'POST',
                                    dataType: 'json',
                                    url: '<?= site_url() ?>master/do_Simpan_Provinsi',
                    ...
                    var handleTable = function () {
                        ...
                        $('#tabelku').dataTable({
                            ...
                            "sServerMethod": "POST",
                            "bRetrieve": true,
                            "sAjaxSource": "<?= site_url() ?>master/do_Tabel_Provinsi",
                        ...
                        $('body').on('click', '#tabelku_wrapper .btn-editable', function () {
                            ...
                            $.ajax({
                                method: 'POST',
                                dataType: 'json',
                                url: '<?= site_url() ?>master/do_Ubah_Provinsi',
                                data: {'id_provinsi': $(this).attr("data-id")},
                                success: function (data) {
                                    if (data.success === true) {
                                        $.unblockUI();
                                        $('#mode_form').val(data.results.mode_form);
                                        $('#id_provinsi').val(data.results.id_provinsi);
                                        $('#nama_provinsi').val(data.results.nama_provinsi);
                                        $('#tambah_provinsi').modal('show');
                                    } else {
                                        $.unblockUI();
                                        toastr.warning(data.msgServer);
                                    }
                        ...
                        $('body').on('click', '#tabelku_wrapper .btn-removable', function () {
                            ...
                            bootbox.dialog({
                                message: "Apakah anda yakin menghapus </br>Provinsi : <b>" + name + "</b> ?",
                                    ...
                                    success: {
                                        ...
                                        callback: function () {
                                            ...
                                            $.ajax({
                                                method: 'POST',
                                                dataType: 'json',
                                                url: '<?= site_url() ?>master/do_Hapus_Provinsi',
                                                data: {'id_provinsi': id},
                                                success: function (data) {
                                                    if (data.success === true) {
                                                        $.unblockUI();
